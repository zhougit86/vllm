...
class TestTritonTopkTopp:
    ...
    def test_noncontiguous_logits_match_contiguous(self, mode: str):
        ...
        base = torch.linspace(10.0, -10.0, vocab_size, dtype=torch.float32)
        source = base[None, :] + (
            torch.arange(batch_size, dtype=torch.float32)[:, None] / 1000.0
        )
        ...
        if mode in ("topk_only", "topk_and_topp"):
            k = torch.full((batch_size,), 154, dtype=torch.int32)
